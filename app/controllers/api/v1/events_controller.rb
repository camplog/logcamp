module Api
  module V1
    class EventsController < Api::BaseController
      ...
      def index
        @events = @current_application.events.order('date DESC').limit(20)
        if params[:q].present?
      ...
      def create
        ...
        if @event.save
          ...
          EventMailer.notify_members(@event).deliver_now if @event.alert?
        else
      ...
      private
        ...
        def safe_params
          params.require(:event).permit(:status, :message, :alert, :date).tap do |white_listed|
